...
public class GuardPost : MonoBehaviour
{
    ...
    private void Update()
    {
        if (guard != null && guardAtPost && guard.GetComponent<SharedEnemyAI>().CheckIfOnDuty() && !guard.GetComponent<SharedEnemyAI>().GetIsDetecting())
        {
            ...
            if (guardAtPost && Quaternion.Angle(guard.transform.rotation, rotationToFront) > 1f)
